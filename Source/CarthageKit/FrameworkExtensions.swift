...
extension URL {
	...
	internal func swiftHeaderURL() -> URL? {
		...
		if let standardHeader = dirContents?.first(where: { $0.absoluteString.contains("-Swift.h") }) {
			return standardHeader
		}

		// Fallback: scan only the first line of header files
		return dirContents?.first(where: { url in
			guard url.pathExtension == "h" else { return false }
			...
			do {
				...
				// Check first line for Swift generation comment
				let firstLine = firstChunk.components(separatedBy: .newlines).first ?? ""
				return firstLine.contains("Generated by Apple Swift version")
			} catch {
				return false
			}
		})
