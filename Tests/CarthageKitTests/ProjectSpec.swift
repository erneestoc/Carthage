...
class ProjectSpec: QuickSpec {
	override func spec() {
		...
		describe("swiftHeaderURL") {
			let tempDirectoryTemplate = (NSTemporaryDirectory() as NSString).appendingPathComponent("SwiftHeaderTest-XXXXXX")
			var tempDirectoryURL: URL!
			var frameworkURL: URL!
			var headersURL: URL!
			
			beforeEach {
				tempDirectoryURL = URL(fileURLWithPath: FileManager.default.reactive.createTemporaryDirectoryWithTemplate(tempDirectoryTemplate).single()!.value!.path)
				frameworkURL = tempDirectoryURL.appendingPathComponent("TestFramework.framework")
				headersURL = frameworkURL.appendingPathComponent("Headers", isDirectory: true)
			...
			it("should find Swift header by standard naming pattern") {
				let standardHeaderURL = headersURL.appendingPathComponent("TestFramework-Swift.h")
				try! "".write(to: standardHeaderURL, atomically: true, encoding: .utf8)
				
				let swiftHeader = frameworkURL.swiftHeaderURL()
				expect(swiftHeader).notTo(beNil())
				expect(swiftHeader?.lastPathComponent) == "TestFramework-Swift.h"
			}
			
			it("should find Swift header by content when standard pattern doesn't match") {
				let nonStandardHeaderURL = headersURL.appendingPathComponent("SwiftHeader.h")
				let headerContent = "// Generated by Apple Swift version 5.10 (swiftlang-5.10.0.13 clang-1500.3.9.4)\n// Non-standard Swift header"
				try! headerContent.write(to: nonStandardHeaderURL, atomically: true, encoding: .utf8)
				
				// Add another header file that isn't a Swift header
				try! "// Regular Objective-C header".write(to: headersURL.appendingPathComponent("RegularHeader.h"), atomically: true, encoding: .utf8)
				
				let swiftHeader = frameworkURL.swiftHeaderURL()
				expect(swiftHeader).notTo(beNil())
				expect(swiftHeader?.lastPathComponent) == "SwiftHeader.h"
			}
			
			it("should return nil when no Swift header is found") {
				try! "// Regular Objective-C header".write(to: headersURL.appendingPathComponent("RegularHeader.h"), atomically: true, encoding: .utf8)
				
				let swiftHeader = frameworkURL.swiftHeaderURL()
				expect(swiftHeader).to(beNil())
			}
		}
	}
}
